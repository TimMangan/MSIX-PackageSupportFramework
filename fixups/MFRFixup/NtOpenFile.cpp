...
)
{
    ...
    try
    {
        //if (guard)
        {
            ...
            Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtOpenFileFixup unguarded and informational", g_MfrModuleName, dllInstance);
            if (ObjectAttributes->ObjectName != NULL)
            {
                Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtOpenFileFixup RootDirectory=0x%x ObjectName=%ls", g_MfrModuleName, dllInstance, ObjectAttributes->RootDirectory, ObjectAttributes->ObjectName->Buffer);
            }
            else
            {
                Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtOpenFileFixup RootDirectory=0x%x ObjectName=NULL", g_MfrModuleName, dllInstance, ObjectAttributes->RootDirectory);
            }
        ...
        Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtOpenFileFixup result=0x%x", g_MfrModuleName, dllInstance, retfinal);
        return retfinal;
    }
    // Fall back to assuming no redirection is necessary if exception
    LOGGED_CATCHHANDLER_MIN(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"NtDll_NtOpenFileFixup")
