    ...
    _In_ DWORD flags) noexcept
{
    ...
    try
    {
        if (guard)
        {
            LogString(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"MoveFileExFixup From", existingFileName);
            LogString(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"MoveFileExFixup To", newFileName);
            Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup with flags 0x%x", g_MfrModuleName, dllInstance, flags);
            ...
            if (AApos != std::wstring::npos)
            {
                ...
                LogString(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"MoveFileExFixup existing adjustment", wExistingFileName.c_str());
            }
            ...
            DetermineCohorts(LogLevel_DebugIntermediate, wExistingFileName, &cohortsExisting, dllInstance, L"MoveFileExFixup (existingFileName)");

            Cohorts cohortsNew;
            DetermineCohorts(LogLevel_DebugIntermediate, wNewFileName, &cohortsNew, dllInstance, L"MoveFileExFixup (newFileName)");
            ...
            if (!MFRConfiguration.Ilv_Aware)
            {
                ...
                Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup: Source      to be is %s", g_MfrModuleName, dllInstance, UseExistingFile.c_str());
                Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup: Destination to be is %s", g_MfrModuleName, dllInstance, UseNewFile.c_str());
                if (ExistingFileIsPackagePath)
                {
                    Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup: ExistingIsInPackagePath", g_MfrModuleName, dllInstance);
                }
                    ...
                    (flags & MOVEFILE_COPY_ALLOWED) != 0)
                {
                    ...
                    Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup: from is %s", g_MfrModuleName, dllInstance, rldUseExistingFile.c_str());
                    Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup:   to is %s", g_MfrModuleName, dllInstance, rldUseNewFile.c_str());
                    retfinal = impl::MoveFileEx(rldUseExistingFile.c_str(), rldUseNewFile.c_str(), flags);
                    if (retfinal == 0)
                    {
                        Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup returns FAILURE 0x%x", g_MfrModuleName, dllInstance, GetLastError());
                    }
                    else
                    {
                        Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup returns SUCCESS 0x%x", g_MfrModuleName, dllInstance, retfinal);
                    }
                    return retfinal;
                }
                else
                {
                    ...
                    {
                        Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup: Implemeting stdcopy from is %s", g_MfrModuleName, dllInstance, rldUseExistingFile.c_str());
                        Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup: Implemeting stdcopy   to is %s", g_MfrModuleName, dllInstance, rldUseNewFile.c_str());
                        ...
                        if (retfinal == 0)
                        {
                            Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup via copy(file) returns FAILURE 0x%x GetLastError 0x%x", g_MfrModuleName, dllInstance, eCode, GetLastError());
                        }
                        else
                        {
                            Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup via copy(file) returns SUCCESS 0x%x", g_MfrModuleName, dllInstance, retfinal);
                        }
                        // TODO: remove old???
                        return retfinal;
                    }
                    else
                    {
                        ...
                        if (retfinal == 0)
                        {
                            Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup via copy(dir) returns FAILURE 0x%x GetLastError 0x%x", g_MfrModuleName, dllInstance, eCode, GetLastError());
                        }
                        else
                        {
                            Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup via copy(dir) returns SUCCESS 0x%x", g_MfrModuleName, dllInstance, retfinal);
                        }
                        // TODO: Remove old???
                        return retfinal;
                    }
            ...
            else
            {
                ...
                PreCreateLocalFoldersIfNeededForWrite(LogLevel_DebugBasic, UseNewFile, cohortsNew.WsPackage, dllInstance, L"MoveFileExFixup");
                ...
                CowLocalFoldersIfNeededForWrite(LogLevel_DebugBasic, UseNewFile, cohortsNew.WsPackage, dllInstance, L"MoveFileExFixup");
                // In a write to package scenario, folders may be needed.
                PreCreatePackageFoldersIfIlvNeededForWrite(LogLevel_DebugBasic, UseNewFile, dllInstance, L"MoveFileExFixup");

               Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup: IlvAware Source      to be is %s", g_MfrModuleName, dllInstance, UseExistingFile.c_str());
               Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup: IlvAware Destination to be is %s", g_MfrModuleName, dllInstance, UseNewFile.c_str());
        ...
        {
        LogString(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"MoveFileExFixup Unguarded From", existingFileName);
        LogString(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"MoveFileExFixup Unguarded To", newFileName);
        }
    }
    // Fall back to assuming no redirection is necessary if exception
    LOGGED_CATCHHANDLER_MIN(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"MoveFileExFixup")
    ...
    Log(LogLevel_DebugBasic, L"[%s%d] MoveFileExFixup returns 0x%x", g_MfrModuleName, dllInstance, retfinal);
    return retfinal;
}
