...
BOOL __stdcall RemoveDirectoryFixup(_In_ const CharT* pathName) noexcept
{
    ...
    try
    {
        if (guard)
        {
            ...
            case mfr::mfr_path_types::in_native_area:
#if MOREDEBUG
                Log(L"[%d] RemoveDirectoryFixup    in_native_area", DllInstance);
#endif
                map = mfr::Find_LocalRedirMapping_FromNativePath_ForwardSearch(file_mfr.Request_NormalizedPath.c_str());
                if (map.Valid_mapping)
                {
#if MOREDEBUG
                    Log(L"[%d] RemoveDirectoryFixup    match on LocalRedirMapping", DllInstance);
#endif
                    ...
                    if (PathExists(testWsRedirected.c_str()))
                    {
                        ...
                        retfinal = WRAPPER_REMOVEDIRECTORY(testWsRedirected, DllInstance, debug);
                ...
                {
#if MOREDEBUG
                    Log(L"[%d] RemoveDirectoryFixup    match on TraditionalRedirMapping", DllInstance);
#endif
...
#if MOREDEBUG
                    Log(L"[%d] RemoveDirectoryFixup      RedirPath=%s", DllInstance, testWsRedirected.c_str());
                    Log(L"[%d] RemoveDirectoryFixup    PackagePath=%s", DllInstance, testWsPackage.c_str());
                    Log(L"[%d] RemoveDirectoryFixup     NativePath=%s", DllInstance, testWsNative.c_str());
#endif
                    if (PathExists(testWsRedirected.c_str()))
                    {
                        retfinal = WRAPPER_REMOVEDIRECTORY(testWsRedirected, DllInstance, debug);
                    ...
                    {
                        // There isn't such a file anywhere.
                        return WRAPPER_REMOVEDIRECTORY(testWsRequested, DllInstance, debug);
                    }
                }
                break;
            case mfr::mfr_path_types::in_package_pvad_area:
#if MOREDEBUG
                Log(L"[%d] RemoveDirectoryFixup    in_package_pvad_area", DllInstance);
#endif
...
#if MOREDEBUG
                Log(L"[%d] RemoveDirectoryFixup    in_package_vfs_area", DllInstance);
#endif
            ...
            case mfr::mfr_path_types::in_redirection_area_writablepackageroot:
#if MOREDEBUG
                Log(L"[%d] RemoveDirectoryFixup    in_redirection_area_writablepackageroot", DllInstance);
#endif
                ...
                if (map.Valid_mapping)
                {
                    ...
                    {
                        retfinal = WRAPPER_REMOVEDIRECTORY(testWsNative, DllInstance, debug);
                        return retfinal;
                    }
                    else
                    {
...
#if MOREDEBUG
                Log(L"[%d] RemoveDirectoryFixup    in_redirection_area_other", DllInstance);
#endif
                if (PathExists(testWsRedirected.c_str()))
                {
                    retfinal = WRAPPER_REMOVEDIRECTORY(testWsRedirected, DllInstance, debug);
    ...
    return impl::RemoveDirectory(LongRemovingDirectory.c_str());
}
