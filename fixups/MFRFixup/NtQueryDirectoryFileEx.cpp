                                    ...
                                    PUNICODE_STRING        FileName)
{
    ...
    try
    {
        ...
        if (thisHandle != INVALID_HANDLE_VALUE)
        {
            ...
            Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup %s alternative %s return status=0x%x", g_MfrModuleName, dllInstance, casetype.c_str(), wThisPathName.c_str(), retfinal);
            CloseHandle(thisHandle);
        }
        else
        {
            Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup %s alternative %s failed to open handle.", g_MfrModuleName, dllInstance, casetype.c_str(), wThisPathName.c_str());
        }
    }
    catch (...)
    {
        Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup %s alternative exception.", g_MfrModuleName, dllInstance, casetype.c_str());
    }
    return retfinal;
}
...
)
{
   ...
   // We need to determine the filepath of that directory, so that we can also search it's cohorts.
    ...
    Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup return status=0x%x", g_MfrModuleName, dllInstance, retfinal);
    ...
    if (retfinal == 0xC000000FL)
    {
        ...
        Cohorts cohorts;
        DetermineCohorts(LogLevel_DebugIntermediate, wfilePath, &cohorts, dllInstance, L"NtDll_NtQueryDirectoryFileExFixup");
    ...
    try
    {
        ...
        case 1: //FILE_DIRECTORY_INFORMATION:
            while (wideData1 != NULL)
            {
                ...
                {
                    Log(LogLevel_DebugIntermediate, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup IoStatusBlock Status 0x%x Attributes=0x%x FromDir=%s FileLen=0x%x File=%s", g_MfrModuleName, dllInstance, IoStatusBlock->Status, wideData1->FileAttributes, DirPathUsed.c_str(), wideData1->FileNameLength, (const wchar_t*)wideData1->FileName);
                }
        ...
        case 2: //FILE_FULL_DIRECTORY_INFORMATION:
            ...
            {
                Log(LogLevel_DebugIntermediate, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup IoStatusBlock Status 0x%x Attributes=0x%x FromDir=%s FileLen=0x%x File=%s", g_MfrModuleName, dllInstance, IoStatusBlock->Status, wideData2->FileAttributes, DirPathUsed.c_str(), wideData2->FileNameLength, (const wchar_t*)wideData2->FileName);
            }
        ...
        case 3: //FILE_BOTH_DIRECTORY_INFORMATION:
            if (wideData3->FileNameLength > 0)
            {
                ...
                Log(LogLevel_DebugIntermediate, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup IoStatusBlock Status 0x%x Attributes=0x%x FromDir=%s FileLen=0x%x File=%s", g_MfrModuleName, dllInstance, IoStatusBlock->Status, wideData3->FileAttributes, DirPathUsed.c_str(), wideData3->FileNameLength, (const wchar_t*)wideData3->FileName);
            }
        ...
        case 12: // FILE_NAMES_INFORMATION
            ...
            {
                Log(LogLevel_DebugIntermediate, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup IoStatusBlock Status 0x%x FromDir=%s FileLen=0x%x File=%s", g_MfrModuleName, dllInstance, IoStatusBlock->Status, DirPathUsed.c_str(), wideData12->FileNameLength, (const wchar_t*)wideData12->FileName);
            }
        ...
        case 29: // FILE_OBJECT_ID:
            Log(LogLevel_DebugIntermediate, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup IoStatusBlock Status 0x%x", g_MfrModuleName, dllInstance, IoStatusBlock->Status);
            break;
        ...
        case 37: //FILE_ID_BOTH_DIR_INFORMATION:
            ...
            {
                Log(LogLevel_DebugIntermediate, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup IoStatusBlock Status 0x%x Attributes=0x%x FromDir=%s FileLen=0x%x File=%s", g_MfrModuleName, dllInstance, IoStatusBlock->Status, wideData37->FileAttributes, DirPathUsed.c_str(), wideData37->FileNameLength, (const wchar_t*)wideData37->FileName);
            }
        ...
        case 38: //FILE_ID_DIR_INFORMATION:
            ...
            {
                Log(LogLevel_DebugIntermediate, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup IoStatusBlock Status 0x%x Attributes=0x%x FromDir=%s FileLen0x%x File=%s", g_MfrModuleName, dllInstance, IoStatusBlock->Status, wideData38->FileAttributes, DirPathUsed.c_str(), wideData38->FileNameLength, (const wchar_t*)wideData38->FileName);
            }
...
)
{
    ...
    try
    {
        if (guard)
        {
            ...
            if (Event != NULL || ApcRoutine != NULL)
            {
                ...
                Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup isAsync not redirected", g_MfrModuleName, dllInstance);
            ...
            else
            {
                ...
                if (FileName != NULL)
                {
                    Log(LogLevel_DebugBasic, L"[%s%d] NtDll_NtQueryDirectoryFileExFixup RootDirectory=0x%x ReqClass=0x%x QueryFlags=%d FileName=%ls", g_MfrModuleName, dllInstance, FileHandle, FileInformationClass, QueryFlags, FileName->Buffer);
                }
                //LogCallingModuleInstance(g_MfrModuleName, dllInstance);
    ...
    LOGGED_CATCHHANDLER_MIN(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"NtDll_NtQueryDirectoryFileExFixup")
