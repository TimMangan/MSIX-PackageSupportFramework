...
int InitializeFindFileTests1()
{
    ...
    MfrFindFileTest t_Native_PFF1 = { "MFR+ILV Native-file VFS single file exists in package", true, true, true,
                                       temp.c_str(),
                                       true, 1,  ERROR_SUCCESS };
    MfrFindFileTests1.push_back(t_Native_PFF1);

    temp = g_NativePF + L"\\PlaceholderTest";
    MfrFindFileTest t_Native_PFF2 = { "MFR+ILV Native-file VFS folder exists in package that has files", true, false, false,
                                       temp.c_str(),
                                       true, 1,  ERROR_SUCCESS };
    MfrFindFileTests1.push_back(t_Native_PFF2);

    temp = g_NativePF + L"\\PlaceholderTest\\MissingPlaceholder.txt";
    MfrFindFileTest t_Native_PFF1E = { "MFR+ILV Native-file VFS single file missing in package (expect fail)", true, false, false,
                                       temp.c_str(),
                                       false, 0,  ERROR_FILE_NOT_FOUND };
    MfrFindFileTests1.push_back(t_Native_PFF1E);

    temp = g_NativePF + L"\\MissingPlaceholderTest_FF1";
    MfrFindFileTest t_Native_PFF2E = { "MFR+ILV Native-file VFS folder missing in package (expect fail)", true, false, false,
                                       temp.c_str(),
    ...
    MfrFindFileTest t_Native_PFF3 = { "MFR+ILV Native-file VFS multiple file exists in package", true, false, false,
                                       temp.c_str(),
                                       true, 2,  ERROR_SUCCESS };
    MfrFindFileTests1.push_back(t_Native_PFF3);

    temp = g_NativePF + L"\\PlaceholderTest2\\*";
    MfrFindFileTest t_Native_PFF4 = { "MFR+ILV Native-file VFS folder with wildcard exists in package with files", true, false, false,
                                       temp.c_str(),
    ...
    MfrFindFileTest t_Package_PFF1 = { "MFR+ILV Package-file VFS single file exists in package", true, true, true,
                                       temp.c_str(),
                                       true, 1,  ERROR_SUCCESS };
    ...
    MfrFindFileTest t_Package_PFF2 = { "MFR+ILV Package-file VFS folder exists in package that has files", true, false, false,
                                       temp.c_str(),
                                       true, 1,  ERROR_SUCCESS };
    MfrFindFileTests1.push_back(t_Package_PFF2);

    temp = VfsPf + L"\\PlaceholderTest\\MissingPlaceholder.txt";
    MfrFindFileTest t_Package_PFF1E = { "MFR+ILV Package-file VFS single file missing in package (expect fail)", true, false, false,
                                       temp.c_str(),
                                       false, 0,  ERROR_FILE_NOT_FOUND };
    MfrFindFileTests1.push_back(t_Package_PFF1E);

    temp = VfsPf + L"\\MissingPlaceholderTest_FF2";
    MfrFindFileTest t_Package_PFF2E = { "MFR+ILV Package-file VFS folder missing in package (expect fail)", true, false, false,
                                       temp.c_str(),
                                       false, 0,  ERROR_PATH_NOT_FOUND };
    ...
    MfrFindFileTest t_Package_PFF3 = { "MFR+ILV Package-file VFS multiple file exists in package", true, false, false,
                                       temp.c_str(),
                                       true, 2,  ERROR_SUCCESS };
    MfrFindFileTests1.push_back(t_Package_PFF3);

    temp = VfsPf + L"\\PlaceholderTest2\\*";
    MfrFindFileTest t_Package_PFF4 = { "MFR+ILV Package-file VFS folder with wildcard exists in package with files", true, false, false,
                                       temp.c_str(),
                                       true, 5,  ERROR_SUCCESS };
    ...
    MfrFindFileTest t_Redirected_PFF1 = { "MFR+ILV Redirected-file VFS single file exists in package", true, true, true,
                                       temp.c_str(),
                                       true, 1,  ERROR_SUCCESS };
    ...
    MfrFindFileTest t_Redirected_PFF2 = { "MFR+ILV Redirected-file VFS folder exists in package that has files", true, false, false,
                                       temp.c_str(),
                                       true, 1,  ERROR_SUCCESS };
    MfrFindFileTests1.push_back(t_Redirected_PFF2);

    temp = WriteRootPF + L"\\PlaceholderTest\\MissingPlaceholder.txt";
    MfrFindFileTest t_Redirected_PFF1E = { "MFR+ILV Redirected-file VFS single file missing in package (expect fail)", true, false, false,
                                       temp.c_str(),
                                       false, 0,  ERROR_FILE_NOT_FOUND };
    MfrFindFileTests1.push_back(t_Redirected_PFF1E);

    temp = WriteRootPF + L"\\MissingPlaceholderTest_FF3";
    MfrFindFileTest t_Redirected_PFF2E = { "MFR+ILV Redirected-file VFS folder missing in package (expect fail)", true, false, false,
                                       temp.c_str(),
                                       false, 0,  ERROR_PATH_NOT_FOUND };
    ...
    MfrFindFileTest t_Redirected_PFF3 = { "MFR+ILV Redirected-file VFS multiple file exists in package", true, false, false,
                                       temp.c_str(),
                                       true, 2,  ERROR_SUCCESS };
    MfrFindFileTests1.push_back(t_Redirected_PFF3);

    temp = WriteRootPF + L"\\PlaceholderTest2\\*";
    MfrFindFileTest t_Redirected_PFF4 = { "MFR+ILV Redirected-file VFS folder with wildcard exists in package with files", true, false, false,
                                       temp.c_str(),
                                       true, 5,  ERROR_SUCCESS };
    ...
    MfrFindFileTest t_Redirected_NET1 = { "MFR+ILV Native-folder present but not in package", true, false, false,
                                       temp.c_str(),
                                       true, 1,  ERROR_SUCCESS };
    ...
    temp = L"C:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\\\*.xsd";
    MfrFindFileTest t_Redirected_NET2 = { "MFR+ILV Native-folder present but not in package (with extra backslash)", true, false, false,
                                       temp.c_str(),
                                       true, 1,  ERROR_SUCCESS };
...
int InitializeFindFileExTests1()
{
    ...
    // Request to Native File/Folder Locations for FindFile for single file
    temp = g_NativePF + L"\\PlaceholderTest\\Placeholder.txt";
    MfrFindFileExTest t_Native_NF1 = { "MFR+ILV Ex: Native-file VFS single file exists in package; basic namematch 0", true, false, false,
    ...
    temp = g_NativePF + L"\\PlaceholderTest\\PLACEHOLDER.txt";
    MfrFindFileExTest t_Native_NF2 = { "MFR+ILV Ex: Native-file VFS single file exists in package; basic namematch casesensitive (normally succeeds anyway)", true, false, false,
    ...
    temp = g_NativePF + L"\\PlaceholderTest\\Placeholder.txt";
    MfrFindFileExTest t_Native_NF3 = { "MFR+ILV Ex: Native-file VFS single file exists in package; standard namematch casesensitive", true, false, false,
    ...
    MfrFindFileExTest t_Native_NF4 = { "MFR+ILV Ex: Native-folder VFS folder exists in package with files standard directorymatch", true, false, false,
    ...
    MfrFindFileExTest t_Native_NF5e = { "MFR+ILV Ex: Native-folder VFS folder missing from package with files standard directorymatch", true, false, false,
                                       temp.c_str(), FINDEX_INFO_LEVELS::FindExInfoStandard, FINDEX_SEARCH_OPS::FindExSearchLimitToDirectories, 0,
                                       false, 0,  ERROR_PATH_NOT_FOUND };
    ...
    temp = VfsPf + L"\\PlaceholderTest\\MissingPlaceholder.txt";
    MfrFindFileExTest t_Package_PFF1E = { "MFR+ILV Ex: Package-file VFS single file missing in package standard namematch (expect fail)", true, false, false,
    ...
    temp = VfsPf + L"\\MissingPlaceholderTest_FF4";
    MfrFindFileExTest t_Package_PFF2E = { "MFR+ILV Ex: Package-file VFS folder missing in package standard namematch (expect fail)", true, false, false,
                                       temp.c_str(), FINDEX_INFO_LEVELS::FindExInfoStandard, FINDEX_SEARCH_OPS::FindExSearchNameMatch, 0,
                                       false, 0,  ERROR_PATH_NOT_FOUND };
    ...
    // Request to Native File/Folder Locations for FindFile for multiple files
    temp = WriteRootPF + L"\\PlaceholderTest2\\Test*";
    MfrFindFileExTest t_Redirected_RF1 = { "MFR+ILV Ex: Redirected-file VFS multiple file exists in package standard namematch", true, false, false,
                                       temp.c_str(), FINDEX_INFO_LEVELS::FindExInfoStandard, FINDEX_SEARCH_OPS::FindExSearchNameMatch, 0,
                                       true, 2,  ERROR_SUCCESS };
    MfrFindFileExTests1.push_back(t_Redirected_RF1);

    temp = WriteRootPF + L"\\PlaceholderTest2\\*";
    MfrFindFileExTest t_Redirected_RF2 = { "MFR+ILV Ex: Redirected-file VFS folder with wildcard exists in package with files standard namematch", true, false, false,
                                       temp.c_str(), FINDEX_INFO_LEVELS::FindExInfoStandard, FINDEX_SEARCH_OPS::FindExSearchNameMatch, 0,
                                       true, 5,  ERROR_SUCCESS };
...
int RunFindFileIndividualTest(MfrFindFileTest testInput, int setnum)
{
    ...
    {
        std::string testname = "MFR+ILV FindFirst/Next File Test set";
...
int RunFindFileExIndividualTest(MfrFindFileExTest testInput, int setnum)
{
    ...
    if (testInput.enabled)
    {
        std::string testname = "MFR+ILV FindFirstEx/Next File Test set";
        testname.append(std::to_string(setnum));
        testname.append(": ");
        testname.append(testInput.TestName);
        test_begin(testname);
