        ...
        if (guard)
        {
            ...
            if (fileName != NULL)
            {
                        ...
                        if (cohorts.map.Valid_mapping)
                        {
                            ...
                            case mfr::mfr_redirect_flags::prefer_redirection_local:
                                ...
                                if (!cohorts.map.IsAnExclusionToRedirect && PathExists(cohorts.WsRedirected.c_str()))
                                {
                                    ...
                                    WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsRedirected, debug);
                                }
                                else if (PathExists(cohorts.WsPackage.c_str()))
                                {
                                    if (Cow(cohorts.WsPackage, cohorts.WsRedirected, dllInstance, L"WritePrivateProfileSectionFixup"))
                                    {
                                        WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsRedirected, debug);
                                    }
                                    else
                            ...
                            case mfr::mfr_redirect_flags::prefer_redirection_if_package_vfs:
                                    ...
                                    PathExists(cohorts.WsNative.c_str()))
                                {
                                    ...
                                    if (cohorts.map.DoesRuntimeMapNativeToVFS)
                                    {
                                        ...
                                        {
                                            WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsNative, debug);
                                        }
                                    }
                                    else
                                    {
                ...
                else
                {
                    ...
                    PreCreateLocalFoldersIfNeededForWrite(UseFile, cohorts.WsPackage, dllInstance, debug, L"WritePrivateProfileSectionFixup");
                    ...
                    CowLocalFoldersIfNeededForWrite(UseFile, cohorts.WsPackage, dllInstance, debug, L"WritePrivateProfileSectionFixup");
                    // In a write to package scenario, folders may be needed.
                    PreCreatePackageFoldersIfIlvNeededForWrite(UseFile, dllInstance, debug, L"WritePrivateProfileSectionFixup");

                    WRAPPER_WRITEPRIVATEPROFILESECTION(UseFile, debug);
                }
            }
            else
            {
