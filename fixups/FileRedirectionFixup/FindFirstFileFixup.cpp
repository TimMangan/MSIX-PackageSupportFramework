    ...
    _In_ DWORD additionalFlags) noexcept try
{
    ...
    if (!guard)
    {
        LogString(LogLevel_DebugBasic, g_FrfModuleName, FindFirstFileExInstance, L"\tFindFirstFileExFixup: for fileName", fileName);
    ...
    LogString(LogLevel_DebugBasic, g_FrfModuleName, FindFirstFileExInstance,L" \tFindFirstFileEx: for fileName", wfileName.c_str());
    Log(LogLevel_DebugBasic, L"[%s%d]\tFindFirstFileEx: InfoLevel=0x%x searchOp=0x%x addtionalFlags=0x%x ", g_FrfModuleName, FindFirstFileExInstance, infoLevelId, searchOp, additionalFlags);
    ...
    {
        Log(LogLevel_DebugBasic, "[%s%d]\tFindFirstFileEx: has slash", g_FrfModuleName, FindFirstFileExInstance);
        // Special case for single separator at beginning of the wPath "/foo.txt"
        if (dirPos == 0)
        {
            auto nextChar = std::exchange(wPath[dirPos + 1], L'\0');
            Log(LogLevel_DebugBasic, "[%s%d]\tdir comes from single char case =%ls", g_FrfModuleName, FindFirstFileExInstance,  wPath.c_str() );
        ...
        else if (std::iswalpha(wPath[0]) && (wPath[1] == ':'))
        {
            ...
            auto nextChar = std::exchange(wPath[dirPos + 1], L'\0');
            Log(LogLevel_DebugBasic, "[%s%d]\tdir comes from driveroot case=%ls", g_FrfModuleName, FindFirstFileExInstance, wPath.c_str() );
            dir = NormalizePath(wPath.c_str(), FindFirstFileExInstance);
            // Avoid bug in NormalizePath causing absolute_path to fail.
            Log(LogLevel_DebugBasic, L"[%s%d]\t\tnormalabs prefix=%ls", g_FrfModuleName, FindFirstFileExInstance, dir.drive_absolute_path);
            ...
            wPath[dirPos + 1] = nextChar;
            Log(LogLevel_DebugBasic, L"[%s%d]\t\tnormalabs postfix=%ls", g_FrfModuleName, FindFirstFileExInstance, dir.drive_absolute_path);
        }
        else
        {
            auto separator = std::exchange(wPath[dirPos], L'\0');
            Log(LogLevel_DebugBasic, "[%s%d]\tdir comes from =%ls", g_FrfModuleName, FindFirstFileExInstance, wPath.c_str() );
            dir = NormalizePath(wPath.c_str(), FindFirstFileExInstance);
    ...
    {
        Log(LogLevel_DebugBasic, "[%s%d]\tFindFirstFileEx: no slash, assume cwd based.", g_FrfModuleName, FindFirstFileExInstance);
        ...
        Log(LogLevel_DebugBasic, "[%s%d]\tFindFirstFileEx: swap to cwd: %ls", g_FrfModuleName, FindFirstFileExInstance,cwd.c_str());
        dir = NormalizePath(cwd.c_str(), FindFirstFileExInstance);
        pattern = wPath.c_str();
        Log(LogLevel_DebugBasic, "[%s%d]\tFindFirstFileEx: no slash, assumed cwd based type=x%x dap=%ls", g_FrfModuleName, FindFirstFileExInstance, psf::path_type(cwd.c_str()),dir.drive_absolute_path);
    }
    Log(LogLevel_DebugBasic, "[%s%d]\tpattern=%ls wPath=%ls", g_FrfModuleName, FindFirstFileExInstance, pattern, wPath.c_str());
    ...
    //}
    Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile requested_path for [2] (from original) is", g_FrfModuleName, FindFirstFileExInstance);
    Log(LogLevel_DebugBasic, g_FrfModuleName, FindFirstFileExInstance, result->requested_path.c_str());
    ...
    Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile redirected_path for [0] (from redirected) is", g_FrfModuleName, FindFirstFileExInstance);
    Log(LogLevel_DebugBasic, g_FrfModuleName, FindFirstFileExInstance, result->redirect_path.c_str());
    ...
    {
        Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile wPath not in package.", g_FrfModuleName, FindFirstFileExInstance);
        std::filesystem::path vfspath = GetPackageVFSPath(wPath.c_str());
        Log(LogLevel_DebugBasic, L"[%s%d]debug FindFirstFile wPath after GetPackageVFSPath.", g_FrfModuleName, FindFirstFileExInstance);
        if (wcslen(vfspath.c_str()) > 0)
        {
            result->package_vfs_path = vfspath.c_str();
            Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile package_vfs_path for [1] (from vfs_path) is", g_FrfModuleName, FindFirstFileExInstance);
            Log(LogLevel_DebugBasic, result->package_vfs_path.c_str());
        }
        else
        {
            result->package_vfs_path = result->redirect_path; //dir.full_path.c_str();
            Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile package_vfs_path for [1] (from vfs_path) is (non AppData) so not applicable", g_FrfModuleName, FindFirstFileExInstance);
            Log(LogLevel_DebugBasic, g_FrfModuleName, FindFirstFileExInstance, result->package_vfs_path.c_str());
        }
    ...
    else
    {
        ...
        //}
        Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile package_vfs_path for [1] (from vfs_path) is not applicable", g_FrfModuleName, FindFirstFileExInstance);
    }
    ...
    if (result->find_handles[0])
    {
        ...
        Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile[0] (from redirected): had results", g_FrfModuleName, FindFirstFileExInstance);
    }
    else
    {
        ...
        Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile[0] (from redirected): no results", g_FrfModuleName, FindFirstFileExInstance);
    }
    ...
    if (foundVFS == std::wstring::npos)
    {
        ...
        {
            Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile[1] (from vfs_path):   had results", g_FrfModuleName, FindFirstFileExInstance);
        }
        else
        {
            result->package_vfs_path.clear();
            Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile[1] (from vfs_path):   no results", g_FrfModuleName, FindFirstFileExInstance);
        }
        if (!result->find_handles[0])
        {
            if constexpr (psf::is_ansi<CharT>)
            {
                if (copy_find_data(*findData, *ansiData))
                {
                    Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile error set by caller", g_FrfModuleName, FindFirstFileExInstance);
                    // NOTE: Last error set by caller
                    return INVALID_HANDLE_VALUE;
                }
            }
    ...
    {
        Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile[2] (from origial):    had results", g_FrfModuleName, FindFirstFileExInstance);
    }
    else
    {
        result->requested_path.clear();
        Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile[2] (from original):   no results", g_FrfModuleName, FindFirstFileExInstance);
    }
    if (!result->find_handles[0] &&
        !result->find_handles[1])
    {
        if (!result->find_handles[2])
        {
            ...
            {
                Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile error 0x%x", g_FrfModuleName, FindFirstFileExInstance, initialFindError);
    ...
    Log(LogLevel_DebugBasic, L"[%s%d] FindFirstFile returns %ls", g_FrfModuleName, FindFirstFileExInstance, result->cached_data.cFileName);
...
catch (...)
{
    ...
    Log(LogLevel_DebugBasic, L"***FindDirstFileExFixup Exception***");
    return INVALID_HANDLE_VALUE;
}
...
BOOL __stdcall FindNextFileFixup(_In_ HANDLE findFile, _Out_ win32_find_data_t<CharT>* findFileData) noexcept try
{
    ...
    if (!guard)
    {
        Log(LogLevel_DebugBasic, g_FrfModuleName, FindNextFileInstance, L"FindNextFileFixup for file.");
    ...
    Log(LogLevel_DebugBasic, L"[%s%d] FindNextFileFixup is against redir  =%ls", g_FrfModuleName, FindNextFileInstance, data->redirect_path.c_str());
    Log(LogLevel_DebugBasic, L"[%s%d] FindNextFileFixup is against pkgVfs =%ls", g_FrfModuleName, FindNextFileInstance, data->package_vfs_path.c_str());
    Log(LogLevel_DebugBasic, L"[%s%d] FindNextFileFixup is against request=%ls", g_FrfModuleName, FindNextFileInstance, data->requested_path.c_str());
    ...
    auto redirectedFileExists = [&](auto filename)
    {
        ...
        {
            Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile redirectedFileExists returns false.", g_FrfModuleName, FindNextFileInstance);
            return false;
        }
        ...
        Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile redirectedFileExists returns %ls", g_FrfModuleName, FindNextFileInstance, data->redirect_path.c_str());
        return result;
    };
    auto vfspathFileExists = [&](auto filename)
    {
        ...
        {
            Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile vfspathFileExists returns false.", g_FrfModuleName, FindNextFileInstance);
            return false;
        }
        ...
        Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile vfspathFileExists returns %ls", g_FrfModuleName, FindNextFileInstance, data->package_vfs_path.c_str());
        return result;
    };
    ...
    if (!data->redirect_path.empty())
    {
        ...
        {
            Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[0] to be checked.", g_FrfModuleName, FindNextFileInstance);
            if (impl::FindNextFile(data->find_handles[0].get(), findFileData))
            {
                Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[0] returns TRUE: %ls", g_FrfModuleName, FindNextFileInstance, data->cached_data.cFileName);
                return TRUE;
            }
            else if (::GetLastError() == ERROR_NO_MORE_FILES)
            {
                Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[0] had FALSE with ERROR_NO_MORE_FILES.", g_FrfModuleName, FindNextFileInstance);
                data->find_handles[0].reset();

                if (data->package_vfs_path.empty() || !data->find_handles[1])
                {
                    Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[1] not in use.", g_FrfModuleName, FindNextFileInstance);
                    if (data->requested_path.empty() || !data->find_handles[2])
                    {
                        Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[2] not in use, so return ERROR_NO_MORE_FILES.", g_FrfModuleName, FindNextFileInstance);
                ...
                if (!redirectedFileExists(data->cached_data.cFileName))
                {
                    ...
                    {
                        Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[0] returns FALSE with last error set by caller", g_FrfModuleName, FindNextFileInstance);
                        // NOTE: Last error set by caller
                        return FALSE;
                    }

                    Log(LogLevel_DebugBasic,  L"[%s%d] FindNextFile[0] returns TRUE with ERROR_SUCCESS and file %ls", g_FrfModuleName, FindNextFileInstance, data->cached_data.cFileName);
                    ::SetLastError(ERROR_SUCCESS);
                    return TRUE;
                }
            }
            else
            {
                ...
                Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[0] returns FALSE", g_FrfModuleName, FindNextFileInstance);
                return FALSE;
            }
        }
    }
    ...
    if (!data->package_vfs_path.empty())
    {
        while (data->find_handles[1])
        {
            if (impl::FindNextFile(data->find_handles[1].get(), findFileData))
            {
                ...
                {
                    Log(LogLevel_DebugBasic, L"[%s%x] FindNextFile[1] returns TRUE with ERROR_SUCCESS and file %ls", g_FrfModuleName, FindNextFileInstance, findFileData->cFileName);
                    ::SetLastError(ERROR_SUCCESS);
                    return TRUE;
                }
            ...
            {
                Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[1] returns FALSE with ERROR_NO_MORE_FILES_FOUND.", g_FrfModuleName, FindNextFileInstance);
                data->find_handles[1].reset();
                // now check [2]
            }
            else
            {
                Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[1] returns FALSE", g_FrfModuleName, FindNextFileInstance);
                // Error due to something other than reaching the end
                return FALSE;
            }
        }
    ...
    if (!data->requested_path.empty())
    {
        while (data->find_handles[2])
        {
            if (impl::FindNextFile(data->find_handles[2].get(), findFileData))
            {
                ...
                {
                    Log(LogLevel_DebugBasic, L"[%s%x] FindNextFile[2] returns TRUE with ERROR_SUCCESS and file %ls", g_FrfModuleName, FindNextFileInstance, findFileData->cFileName);
                    ::SetLastError(ERROR_SUCCESS);
                    return TRUE;
                }
                // Otherwise, skip this file and check the next one
            ...
            {
                Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[2] returns FALSE with ERROR_NO_MORE_FILES_FOUND.", g_FrfModuleName, FindNextFileInstance);
            ...
            {
                Log(LogLevel_DebugBasic, L"[%s%d] FindNextFile[2] returns FALSE", g_FrfModuleName, FindNextFileInstance);
                // Error due to something other than reaching the end
                return FALSE;
            }
        }
...
BOOL __stdcall FindCloseFixup(_Inout_ HANDLE findHandle) noexcept
{
    ...
    if (!guard)
    {
        Log(LogLevel_DebugBasic, L"FindCloseFixup");
