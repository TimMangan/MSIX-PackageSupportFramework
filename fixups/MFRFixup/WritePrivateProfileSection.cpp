...
#define WRAPPER_WRITEPRIVATEPROFILESECTION(theDestinationFilename) \
    { \
        ...
        Log(LogLevel_DebugBasic, L"[%s%d] WritePrivateProfileSection returns %d on file %s", g_MfrModuleName, dllInstance, retfinal, LongDestinationFilename.c_str()); \
        return retfinal; \
    }
    ...
    _In_opt_ const CharT* fileName) noexcept
{
    ...
    try
    {
        if (guard)
        {
            ...
            if (fileName != NULL)
            {
                LogString(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"WritePrivateProfileSectionFixup for fileName", fileName);
                ...
                Cohorts cohorts;
                DetermineCohorts(LogLevel_DebugIntermediate, wfileName, &cohorts, dllInstance, L"WritePrivateProfileSectionFixup");
                        ...
                        if (cohorts.map.Valid_mapping == mfr::mfr_enabled_types::enabled)
                        {
                            ...
                            case mfr::mfr_redirect_flags::prefer_redirection_local:
                                ...
                                if (cohorts.map.IsAnExclusionToRedirect == mfr::mfr_exclusion_types::not_excluded && PathExists(cohorts.WsRedirected.c_str()))
                                {
                                    ...
                                    WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsRedirected);
                                }
                                else if (PathExists(cohorts.WsPackage.c_str()))
                                {
                                    if (Cow(LogLevel_DebugBasic, cohorts.WsPackage, cohorts.WsRedirected, dllInstance, L"WritePrivateProfileSectionFixup"))
                                    {
                                        WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsRedirected);
                                    }
                                    else
                                    {
                                        WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsPackage);
                                    }
                                }
                                else
                                {
                                    ...
                                    PreCreateFolders(cohorts.WsRedirected.c_str(), dllInstance, L"WritePrivateProfileSectionFixup");
                                    WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsRedirected);
                                }
                                break;
                            ...
                            case mfr::mfr_redirect_flags::prefer_redirection_if_package_vfs:
                                    ...
                                    PathExists(cohorts.WsNative.c_str()))
                                {
                                    ...
                                    {
                                        if (Cow(LogLevel_DebugBasic, cohorts.WsNative, cohorts.WsRedirected, dllInstance, L"WritePrivateProfileSectionFixup"))
                                        {
                                            WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsRedirected);
                                        }
                                        else
                                        {
                                            WRAPPER_WRITEPRIVATEPROFILESECTION(cohorts.WsNative);
                                        }
                                    }
                                    else
                                    {
                ...
                else
                {
                    ...
                    PreCreateLocalFoldersIfNeededForWrite(LogLevel_DebugBasic, UseFile, cohorts.WsPackage, dllInstance, L"WritePrivateProfileSectionFixup");
                    ...
                    CowLocalFoldersIfNeededForWrite(LogLevel_DebugBasic, UseFile, cohorts.WsPackage, dllInstance, L"WritePrivateProfileSectionFixup");
                    // In a write to package scenario, folders may be needed.
                    PreCreatePackageFoldersIfIlvNeededForWrite(LogLevel_DebugBasic, UseFile, dllInstance, L"WritePrivateProfileSectionFixup");

                    WRAPPER_WRITEPRIVATEPROFILESECTION(UseFile);
                }
            }
            else
            {
                Log(LogLevel_DebugBasic, L"[%s%d] WritePrivateProfileSectionFixup: null fileName, don't redirect", g_MfrModuleName, dllInstance);
            }
        }
    }
    // Fall back to assuming no redirection is necessary if exception
    LOGGED_CATCHHANDLER_MIN(LogLevel_DebugBasic, g_MfrModuleName, dllInstance, L"WritePrivateProfileSectionFixup")
