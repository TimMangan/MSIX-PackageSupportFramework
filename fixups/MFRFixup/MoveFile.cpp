...
#ifdef _M_IX86
#pragma comment(linker, "/EXPORT:MoveFileFixupAnsi_Fixup=impl::_MoveFileW.ansi")  // A test to see if exporting these names helps ProcessMonitor stack traces.
#pragma comment(linker, "/EXPORT:MoveFileFixupWide_Fixup=impl::_MoveFileW.wide")  // A test to see if exporting these names helps ProcessMonitor stack traces.
#else
#pragma comment(linker, "/EXPORT:MoveFileFixupAnsi_Fixup=impl::MoveFileW.ansi")  // A test to see if exporting these names helps ProcessMonitor stack traces.
#pragma comment(linker, "/EXPORT:MoveFileFixupWide_Fixup=impl::MoveFileW.wide")  // A test to see if exporting these names helps ProcessMonitor stack traces.
#endif
...
#if MOREDEBUG
                Log(L"[%d] MoveFileFixup: IlvAware Source      to be is %s", dllInstance, UseExistingFile.c_str());
                Log(L"[%d] MoveFileFixup: IlvAware Destination to be is %s", dllInstance, UseNewFile.c_str());
#endif
